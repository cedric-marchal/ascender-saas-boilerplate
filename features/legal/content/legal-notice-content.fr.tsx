type LegalNoticeContentFrProps = {
  appName: string;
};

/**
 * Legal body copy is long, rarely changes, and reads far better as native
 * JSX (headings, lists, links) than as deeply-nested ICU message keys. Per
 * the Phase 2 plan amendment, long legal texts are extracted as per-locale
 * content components instead of `next-intl` rich text — the page component
 * picks the right one via a `Record<Locale, Component>` lookup (see
 * `legal-notice-page.tsx`), never a `locale === "fr"` branch.
 */
function LegalNoticeContentFr({ appName }: LegalNoticeContentFrProps) {
  return (
    <div className="prose prose-zinc dark:prose-invert max-w-none">
      <section>
        <h2>Éditeur du site</h2>
        <p>
          Le site <strong>{appName}</strong> est édité par :
        </p>
        <ul>
          <li>
          ...
          <li>
            <strong>Numéro de TVA :</strong> [Numéro TVA intracommunautaire]
          </li>
          <li>
      ...
      <section>
        ...
        <p>
          L'ensemble des contenus présents sur le site {appName} (textes,
          images, graphismes, logo, icônes, sons, logiciels, etc.) est protégé
      ...
      <section>
        ...
        <p>
          ...
          informations diffusées sur le site, dont il se réserve le droit de
      ...
      <section>
        ...
        <p>
          La création de liens hypertextes vers le site {appName} est soumise à
          l'accord préalable de l'éditeur. Pour toute demande, veuillez nous
      ...
      <section>
        ...
        <p>
          ...
          pouvez nous contacter à l'adresse suivante : contact@example.com
        </p>
      </section>
    </div>
  );
